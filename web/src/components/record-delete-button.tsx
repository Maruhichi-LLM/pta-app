...
type Props = {
  recordId: number;
  recordCaption?: string | null;
};

export function RecordDeleteButton({ recordId, recordCaption }: Props) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-w-md w-full rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-zinc-600">
              {recordCaption ? `「${recordCaption}」` : "この記録"} を削除しますか？
            </p>
            <p className="mt-2 text-sm font-semibold text-red-600">
              この操作は取り消せません。写真データも削除されます。
            </p>
